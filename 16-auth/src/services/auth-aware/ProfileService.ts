...
export default class ProfileService extends AuthAware{
    ...
    async edit(id: string, draft: PostDraft): Promise<Post> {
        const response = await this.axiosInstance.patch<Post>(`${import.meta.env.VITE_REST_SERVER_URL}/profile/${id}`, draft)
        return response.data
    }
}
